...
def _context_text(profile_text: str, city_profile: str | None) -> str:
...
def mutate_profile(profile: object, rng: np.random.Generator):
    mutations = [
        {
            "age": 16,
        ...
        },
        {
            ...
            "job": "elementary worker",
        },
    ]
    return profile.model_copy(update=mutations[int(rng.integers(0, len(mutations)))])
...
def build_training_pairs(
    ...
    *,
    ...
    if not 0.0 <= mutation_ratio <= 1.0:
        raise ValueError("mutation_ratio must be between 0 and 1")

    rng = np.random.default_rng(seed)
    pairs: list[TrainingPair] = []
    for _i in range(sample_size):
        original = profiles[int(rng.integers(len(profiles)))]
        use_mutation = bool(rng.random() < mutation_ratio)
        profile = mutate_profile(original, rng) if use_mutation else original
        profile_text = profile_to_narrative(profile, include_transport=False)
        pairs.append(
            TrainingPair(
                profile_uid=int(profile.uid),
                variant="mutated" if use_mutation else "original",
                profile_text=profile_text,
                context_text=_context_text(profile_text, city_profile),
...
def alignment_prompt(pair: TrainingPair) -> str:
    return (
        "Assess whether this synthetic agent profile is demographically coherent. "
        "Return strictly valid JSON with the keys in this order: reason, score. "
        "The score must be a number from 0 to 1, where 0 means impossible or "
        "highly inconsistent and 1 means fully plausible. Penalize combinations "
        "such as teenagers with graduate degrees or senior occupations, implausible "
        "household-age pairings, and inconsistent age/education/job combinations. "
        "Do not score transport ownership.\n\n"
        f"Profile context:\n{pair.context_text}\n\n"
        f"Candidate label:\n{pair.candidate_text}"
    )
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--city-profile")
    ...
    parser.add_argument("--dataset-output", default="data/profile_coherence_scores.parquet")
    parser.add_argument("--output-model-path", default="models/modernbert-profile-coherence-aligner")
