...
# into the project virtualenv with maturin. Mirrors update_local_fastmob.sh.
#
# The extension path-depends on ../fastmob/fastmob-core with default-features = false,
# so it does NOT pull in the C/SIMD crates (numkong/wass). If you also changed the
# fastmob Rust core, run scripts/update_local_fastmob.sh first.

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
VENV_DIR="${ROOT_DIR}/.venv"
FASTMOB_CORE_DIR="${ROOT_DIR}/../fastmob/fastmob-core"
...
require_path "${FASTMOB_CORE_DIR}/Cargo.toml" "fastmob-core path dependency"
