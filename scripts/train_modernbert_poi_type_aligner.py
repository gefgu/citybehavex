#!/usr/bin/env python
"""Fine-tune a ModernBERT CrossEncoder to score POI-type fit for OTHER-purpose blocks.

Mirrors train_modernbert_activity_aligner.py's shape, but scores "which kind of
public place (semantic cluster) fits this person and schedule block" instead of
"which micro-activity fits" -- the same query/candidate text functions the
zero-shot citybehavex.activities.alignment.score_poi_type_alignment path already
uses at inference time (poi_type_query_text, poi_type_candidate_text), now with a
dedicated fine-tune instead of reusing the micro-activity aligner's checkpoint.
"""
...
def build_training_pairs(
    ...
    *,
    ...
    blocks: list[ActivityBlock] = [
        block for block in diary_activity_blocks(diaries) if block.purpose == "OTHER"
    ]
    if not blocks:
        raise ValueError("diaries contain no OTHER-purpose blocks")

    poi_data: PoiSemanticActivityData = build_poi_semantic_activity_data()
    examples = example_categories_by_semantic_cluster(poi_data)
    semantic_clusters = poi_data.semantic_clusters
    ...
    for i in range(sample_size):
        ...
        semantic_cluster = semantic_clusters[i % len(semantic_clusters)]
        ...
        pairs.append(
            TrainingPair(
                ...
                context_text=poi_type_query_text(profile_text, block),
                candidate_text=poi_type_candidate_text(
                    semantic_cluster, examples.get(semantic_cluster, [])
                ),
            )
        )
    return pairs
...
def alignment_prompt(pair: TrainingPair) -> str:
    return (
        "Assess whether this candidate public-place type fits the person and "
        "schedule block described. Return strictly valid JSON with the keys in "
        "this order: reason, score. The score must be a number from 0 to 1, "
        "where 0 means incompatible and 1 means highly fitting.\n\n"
        "Scoring guidance: consider what kind of public place someone with this "
        "profile would plausibly visit during this schedule block. Do not just "
        "pick the most generic or most common place type -- reward specificity "
        "when it fits, and penalize implausible combinations.\n\n"
        f"Context:\n{pair.context_text}\n\n"
        f"Candidate place type:\n{pair.candidate_text}"
    )
...
def _pair_row(pair: TrainingPair, score: float) -> dict[str, Any]:
    return {
        ...
        "semantic_cluster": pair.semantic_cluster,
        ...
        try:
            payload = post_openai_chat_json(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a strict JSON scorer for public-place-type fit.",
                    },
            ...
            last_error = exc
        raise RuntimeError(
            f"failed to label profile={pair.profile_uid} diary={pair.diary_id} "
            f"block={pair.block_index} cluster={pair.semantic_cluster}: {last_error}"
        ) from last_error
...
def train_cross_encoder(
    ...
    *,
    ...
    examples = [
        InputExample(texts=[row.context_text, row.candidate_text], label=float(row.score))
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--dataset-output", default="data/poi_type_alignment_scores.parquet")
    parser.add_argument("--output-model-path", default="models/modernbert-poi-type-aligner")
