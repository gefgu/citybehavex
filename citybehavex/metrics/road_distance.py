...
fastmob's ``jump_lengths``/``radius_of_gyration`` measure straight-line
...
def jump_lengths_km(
    ...
    *,
    ...
    ``fastmob.jump_lengths(merge=True)``'s sort key and its inclusion of
...
def radius_of_gyration_km(
    ...
    *,
    ...
    their stops -- mirrors fastmob's unweighted-centroid formula
    (``r_g(u) = sqrt(mean(d(r_i, r_cm)^2))``), but measures ``d`` along the
    road network instead of straight-line. Returns a
    ``DataFrame[[uid_col, "radius_of_gyration"]]`` matching fastmob's shape.
    """
    df = _as_polars(df)
    if df.is_empty():
        return pl.DataFrame(
