...
SCRIPT_PATH = Path(__file__).resolve().parents[1] / "scripts" / "train_modernbert_schedule_aligner.py"
SPEC = importlib.util.spec_from_file_location("train_modernbert_schedule_aligner", SCRIPT_PATH)
...
def test_build_training_pairs_stratifies_day_types():
    pairs = aligner.build_training_pairs(
        [_profile(1), _profile(2)],
        [
            ("weekday", _diary("wd", "WORK", "09:00", "17:00")),
            ("weekend", _diary("we", "OTHER", "12:00", "18:00")),
        ],
        sample_size=6,
        seed=7,
    )
