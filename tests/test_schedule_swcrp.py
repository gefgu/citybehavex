...
def test_hard_weekday_weekend_filter():
    ...
    (_, _, _, _, _), chosen, _info = build_sw_crp_diary(
...
def test_determinism():
    ...
    day_types = _calendar_day_types(MONDAY, 7)
    (_, _, _, _, _), c1, _info1 = build_sw_crp_diary(bank, MONDAY, 7, day_types, 30, 7, params)
    (_, _, _, _, _), c2, _info2 = build_sw_crp_diary(bank, MONDAY, 7, day_types, 30, 7, params)
...
def test_diary_arrays_shapes_and_mask():
    ...
    (ts, locs, starts, ends, block_ids), chosen, _info = build_sw_crp_diary(
...
def test_profile_similarity_biases_selection():
    ...
    _, chosen, _info = build_sw_crp_diary(
...
def test_precomputed_alignment_scores_override_embedding_similarity():
    ...
    (_, _, _, _, _), chosen, info = build_sw_crp_diary(
        bank,
        MONDAY,
        1,
        ["weekday"],
...
def test_no_profile_embeddings_uses_popularity():
    ...
    day_types = _calendar_day_types(MONDAY, 7)
    (_, _, _, _, _), chosen, _info = build_sw_crp_diary(bank, MONDAY, 7, day_types, 200, 9, params)
