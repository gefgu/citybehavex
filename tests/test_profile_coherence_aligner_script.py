...
def test_mutate_profile_preserves_identity_spatial_and_transport_fields():
    profile = _profile()
    mutated = aligner.mutate_profile(profile, aligner.np.random.default_rng(2))

    assert mutated.uid == profile.uid
    assert mutated.home_tile == profile.home_tile
    assert mutated.work_tile == profile.work_tile
    assert mutated.has_car == profile.has_car
    assert mutated.has_bike == profile.has_bike
    assert (
        mutated.age != profile.age
        or mutated.education != profile.education
        or mutated.household != profile.household
        or mutated.job != profile.job
    )


def test_build_training_pairs_includes_original_and_mutated_without_transport_leak():
    ...
    assert {pair.variant for pair in pairs} == {"original", "mutated"}
    assert all(pair.candidate_text == COHERENCE_CANDIDATE_TEXT for pair in pairs)
...
def test_label_pairs_does_not_persist_reason(monkeypatch):
    ...
    assert df.loc[0, "variant"] == "original"
...
def test_cli_defaults_point_to_profile_coherence_aligner():
    ...
    assert args.dataset_output == "data/profile_coherence_scores.parquet"
    assert args.output_model_path == "models/modernbert-profile-coherence-aligner"
    assert args.sample_size == 2000
    assert args.mutation_ratio == 0.5
