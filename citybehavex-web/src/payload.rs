...
pub fn chart_section_payload(
    ...
    match section {
        "distributions" => {
            crate::comparison::sections::distributions::distributions_section_payload(ctx, filter_key)
        }
        ...
        "activity" => crate::comparison::sections::activity::activity_section_payload(ctx, filter_key),
        "mobility-laws" => {
            crate::comparison::sections::mobility_laws::mobility_laws_section_payload(ctx, filter_key)
        }
        "micro-activity" => {
            crate::comparison::sections::micro_activity::micro_activity_section_payload(ctx, filter_key)
        }
        "time-use" => crate::comparison::sections::time_use::time_use_section_payload(ctx, filter_key),
...
/// Mirrors `web/backend/app/payload/sections.py::build_metrics_export_payload`,
...
pub fn metrics_export_artifact(ctx: &ComparisonContext) -> anyhow::Result<Value> {
        ...
        match &mut merged {
            ...
            Some(base) => {
                for key in ["cpc", "time_use"] {
                    ...
                    rows.extend(payload["metrics"][key].as_array().cloned().unwrap_or_default());
    ...
    if let Some(base) = &mut merged {
        ...
        for payload in &regular_payloads {
            for row in payload["metrics"]["jsd"].as_array().cloned().unwrap_or_default() {
...
pub(crate) fn choose_filter(ctx: &ComparisonContext, filter_key: &str) -> anyhow::Result<FilterMeta> {
...
fn validation_block(
    ...
    let source = network_block_from_edges(node_count, edge_from, edge_to, source_sidecar, source_kind);
    let source_metrics = source.get("_metric_cache").cloned().unwrap_or_else(|| json!({}));
    ...
    let random_metrics = random.get("_metric_cache").cloned().unwrap_or_else(|| json!({}));
    ...
    source_public.as_object_mut().map(|o| o.remove("_metric_cache"));
    random_public.as_object_mut().map(|o| o.remove("_metric_cache"));
...
fn synthetic_social_graph(social_path: &std::path::Path) -> anyhow::Result<(usize, Vec<u32>, Vec<u32>, Value)> {
    let data: Value = serde_json::from_slice(&std::fs::read(social_path)?)?;
    let node_count = data
        .get("node_count")
        .and_then(Value::as_u64)
        .or_else(|| data.get("nodes").and_then(Value::as_array).map(|v| v.len() as u64))
...
pub fn network_validation_payload(
    ...
    if synthetic_enabled {
        match social_path.filter(|p| p.exists()) {
            None => warnings.push("synthetic_vs_random: social network sidecar not found".to_string()),
