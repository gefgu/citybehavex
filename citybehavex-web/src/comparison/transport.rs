...
pub fn synthetic_transport_leg_records(
    ...
    let work = lf
        ...
        .drop_nulls(Some(cols([
            "uid", "stop_id", "seq", "lat", "lng", "t", "mode",
        ])))
    ...
    let finite = work
        ...
        .with_columns([super::util::haversine_km_expr(
            col("_prev_lat"),
            col("_prev_lng"),
            col("lat"),
            col("lng"),
    ...
    let legs = legs
        ...
        .filter(
            col("_n")
                .gt_eq(lit(2))
                .and(col("_valid_n").fill_null(lit(0)).gt_eq(lit(2))),
        );
    ...
    let legs = join_normalized_mode(legs.lazy(), "_raw_mode", mode_map)?
        .with_columns([((col("_t_max") - col("_t_min"))
            .dt()
            .total_seconds()
...
pub fn observed_transport_leg_records(
    ...
    let uid = uid_col
        .map(str::to_string)
        .or_else(|| detect_in(&col_names, UID_CANDIDATES));
    let dt = datetime_col
        .map(str::to_string)
        .or_else(|| detect_in(&col_names, DATETIME_CANDIDATES));
    let lat = lat_col
        .map(str::to_string)
        .or_else(|| detect_in(&col_names, LAT_CANDIDATES));
    let lng = lng_col
        .map(str::to_string)
        .or_else(|| detect_in(&col_names, LNG_CANDIDATES));
    let mode_col = transport_col
        .map(str::to_string)
        .or_else(|| detect_in(&col_names, TRANSPORT_CANDIDATES));
    ...
    if !missing.is_empty() {
        anyhow::bail!(
            "observed transport comparison missing columns: {}",
            missing.join(", ")
        );
    }
    let (uid, dt, lat, lng, mode_col) = (
        uid.unwrap(),
        dt.unwrap(),
        lat.unwrap(),
        lng.unwrap(),
    ...
    work_df = if let Some(dur) = dur {
        ...
        work_df
            .lazy()
            .with_columns([((col(&dt) - col("_prev_t"))
                .dt()
                .total_seconds()
                .cast(DataType::Float64)
                / lit(60.0))
...
pub fn transport_spatial_summary(
    ...
    for source in sources {
        ...
        for mode in modes {
            ...
            let durations = mode_df
                .column("duration_min")?
                .as_materialized_series()
                .drop_nulls();
            let mean_duration_min = if durations.len() > 0 {
                durations.mean()
            } else {
                None
            };
...
mod tests {
    ...
    fn normalize_mode_lowercases_and_maps() {
        ...
        assert_eq!(
            normalize_transport_mode(Some("  Walk  "), &map),
            Some("walk".to_string())
        );
    ...
    fn default_synthetic_moving_path_appends_suffix() {
        let p =
            default_synthetic_moving_path(Path::new("data/gparis/results/trajectories.parquet"));
        assert_eq!(
            p,
            PathBuf::from("data/gparis/results/trajectories_moving.parquet")
        );
    }
    ...
    fn gparis_moving_sidecar_matches_python_reference() {
        ...
        let expected = [
            (
                "walk",
                10291i64,
                27.128697210945322f64,
                0.2996713489659243f64,
                3.7459041881255457f64,
            ),
            (
                "bike",
                3010,
        ...
        for (row, (mode, count, percent, mean_jump_km, mean_duration_min)) in
            syn.modes.iter().zip(expected.iter())
        {
            assert_eq!(&row.mode, mode);
            assert_eq!(row.count, *count);
            assert!(
                (row.percent - percent).abs() < 1e-6,
                "{mode} percent: got {}",
                row.percent
            );
            assert!(
