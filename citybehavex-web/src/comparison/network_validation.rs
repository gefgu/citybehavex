...
pub fn resolve_observed_location(
    ...
    if matches!(location_mode, LocationMode::LocationCol) && chosen.is_none() {
        anyhow::bail!("network validation location_col not found: {:?}", location_col);
    }
    ...
    let lat_col = detect_in(&cols, columns::LAT_CANDIDATES)
        .ok_or_else(|| anyhow::anyhow!("h3 network validation requires latitude/longitude columns"))?;
    let lng_col = detect_in(&cols, columns::LNG_CANDIDATES)
        .ok_or_else(|| anyhow::anyhow!("h3 network validation requires latitude/longitude columns"))?;
...
pub fn observed_edges_and_persistence(
    ...
    let uid_series = df.column(uid_col)?.as_materialized_series().cast(&DataType::String)?;
    ...
    let (edge_from, edge_to, persistence, skipped_groups, skipped_rows) =
        build_co_presence_edges(&day_codes, &location_codes, &nodes, max_group_size, time_steps);
...
mod tests {
    ...
    fn location_col_mode_errors_when_column_missing() {
        let df = sample_df();
        let err = resolve_observed_location(&df, LocationMode::LocationCol, Some("nope"), 9)
            .unwrap_err();
    ...
    fn max_group_size_below_two_errors() {
        ...
        .unwrap_err();
        assert!(err.to_string().contains("max_group_size must be at least 2"));
    }
}
