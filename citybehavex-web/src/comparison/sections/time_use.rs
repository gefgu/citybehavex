...
//! Mirrors `web/backend/app/payload/legacy.py`'s time-use section.
...
pub fn time_use_section_payload(ctx: &ComparisonContext, filter_key: &str) -> anyhow::Result<Value> {
...
mod tests {
    ...
    fn time_use_metric_rows_average_absolute_share_difference() {
        ...
        assert_eq!(rows[0]["metric_name"], "Mean absolute time-use share difference");
