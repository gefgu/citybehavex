...
/// in minutes).
fn stvd_emd_distribution(layer: &crate::comparison::stvd::HourlyLayer) -> (Vec<f64>, Vec<f64>, Vec<f64>, Vec<f64>) {
    let mut xs = Vec::new();
    let mut ys = Vec::new();
    let mut ts = Vec::new();
    let mut ws = Vec::new();
...
pub fn metrics_section_payload(
    ...
    use crate::payload::{choose_filter, duration_col, empty_chart_payload, prepared_visits_for_filter};
    ...
    if let Some(obs_traj) = &observed_traj {
        let synth_filtered =
            filter_df(&synthetic_traj.df, Some(&synthetic_traj.datetime_col), &filter)?;
    ...
    if let (Some(syn_v), Some(obs_v)) = (visits.synthetic.as_ref(), visits.observed.as_ref()) {
        ...
        let mut labels: Vec<String> = counts_syn.keys().chain(counts_obs.keys()).cloned().collect();
        labels.sort();
        labels.dedup();
        ...
        if let Some(row) = crate::comparison::metrics::jensen_shannon_divergence(&flat_syn, &flat_obs)
            .ok()
        ...
        if let Some(row) = crate::comparison::metrics::time_bin_matrix_jsd(&syn_daily_aligned, &obs_daily_aligned)
            .ok()
        ...
        crate::comparison::sections::time_use::time_use_section_payload(ctx, filter_key)
    {
            ...
            .is_some_and(Value::is_null)
        {
            ...
            payload["metrics"]["time_use"] = crate::comparison::sections::time_use::time_use_metric_rows(
