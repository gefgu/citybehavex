...
def get_timeline_agent_crp(
    ...
    """SW-CRP diary-selection state for one agent: T_a, alpha_a, and per-diary
    ...
    if selected.crp_path.exists():
        ...
        if rows:
            ...
            warnings.append("uid not found in SW-CRP diary selection data")
    else:
        warnings.append("no SW-CRP diary selection data available for this run")
