...
feeding the results into ``fastmob_vis`` widgets and writing an HTML file, it emits
...
from fastmob_vis._core import compute_ecdf
...
# fastmob_vis.stvd.STVD_COLORS so the map reads identically to the HTML report.
STVD_COLORS = [
...
def _traj_like(source: fastmob.TrajDataFrame, df: pl.DataFrame) -> fastmob.TrajDataFrame:
...
def _stvd_emd_distribution(hourly: pl.DataFrame) -> pl.DataFrame:
    ...
    if not rows:
        return pl.DataFrame({"time_bin": [], "mean_volume": [], "lat": [], "lng": []})
    return pl.DataFrame(rows)
    ...
    def distribution_group(meta: dict[str, Any]) -> dict[str, Any]:
        ...
        if real_metric_group_df is not None and real_group_traj is not None:
            for row in (
                ...
                _metric_row(meta, "Visits per user", wasserstein_distance(
                    synth_visits_count, real_visits_count
                ), "visits") if real_visits_count else None,
    ...
    if (wants("activity") or wants("metrics")) and synthetic_visits is not None:
        def _activity_group(meta: dict[str, Any]):
            ...
            if obs_v is not None and not obs_v.is_empty():
                ...
                # fastmob's transition matrix embeds activity labels in an
                # explicit "activity" column -- split them out before the
                # JSD call, which wants a pure numeric matrix plus
                # categories (see _split_transition_matrix_categories).
                synth_trans_num, synth_trans_cats = _split_transition_matrix_categories(synth_transition)
                real_trans_num, real_trans_cats = _split_transition_matrix_categories(real_transition)
                jsd.extend([
                    row for row in (
                        _metric_row(meta, "Activity distribution", _activity_distribution_jsd(syn_v, obs_v)),
                        _metric_row(meta, "Activity transitions", _transition_matrix_jsd(
                            synth_trans_num, synth_trans_cats,
                            real_trans_num, real_trans_cats,
                        )),
                    ) if row is not None
                ])
                real_daily, real_cats, real_bins = daily_activity_distribution(obs_v)
                jsd_row = _metric_row(meta, "Daily activity profile", _time_bin_matrix_jsd(
                    synth_daily, synth_cats, real_daily, real_cats
                ))
    ...
    if wants("metrics") and mode == "comparison" and real_traj is not None:
        for meta in filters:
            def _cpc(meta=meta):
                ...
                if synth_df.is_empty() or real_group_df.is_empty():
                    return []
                return trajectory_common_part_of_commuters_multi(
                    _traj_like(traj, synth_df),
                    _traj_like(real_traj, real_group_df),
...
def _matrix_to_categories(matrix: Any) -> tuple[list[str], np.ndarray]:
    # fastmob's activity_transition_matrix embeds activity labels in an
...
def _motif_distribution(visits: pl.DataFrame):
    return motif_distribution(
        daily_motifs(
            ...
            uid_col="uid", location_col="location_id", purpose_col="purpose",
            datetime_col="start_timestamp", end_datetime_col="end_timestamp",
        ),
