...
/// Mirrors `legacy Python backend/timeline_data.py::query_activity_at_stop`: the
...
/// Mirrors `legacy Python backend/timeline_data.py::query_agent_encounters`: the
...
/// Mirrors `legacy Python backend/api/timeline.py`'s `/timeline/meta` handler body
...
/// Mirrors `legacy Python backend/api/timeline.py`'s `/timeline/agents/{uid}`
/// handler body.
        ...
        for encounter in &mut encounters {
            ...
            match query_profile(exp.profiles_path.as_deref(), contact_uid)? {
                Some(contact_profile) => {
                    ...
                    encounter["contact_narrative"] =
                        contact_narrative.map(Value::from).unwrap_or(Value::Null);
...
/// Mirrors `legacy Python backend/api/timeline.py`'s `/timeline/agents/{uid}/crp`
/// handler body.
...
pub async fn timeline_agent_crp_route(
    ...
    let (exp, run) = selected(&exp_id, q.run.as_deref())?;
    let payload =
        build_agent_crp_payload(&exp, &run, uid).map_err(|e| ApiError::internal(e.to_string()))?;
    Ok(ApiResponse::new(payload))
}

/// Mirrors `legacy Python backend/api/timeline.py`'s `/timeline/agents/{uid}/social`
/// handler body.
