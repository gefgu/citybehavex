...
//! `legacy Python backend/models.py::ApiResponseWrapper`. Every route handler in
...
/// legacy FastAPI's default `HTTPException` JSON shape (`{"detail": "..."}`), so
/// error responses are wire-compatible with what the frontend already
/// expects from the legacy Python backend.
pub struct ApiError {
