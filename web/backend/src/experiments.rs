...
//! Mirrors `legacy Python backend/experiments.py`.
//!
...
/// validation when the legacy Python backend re-reads a config this Rust backend
...
pub fn update_experiment(
    ...
    // the legacy Python backend: a PATCH that only touches `simulation`/
...
mod yaml_quoting_tests {
    ...
    fn round_trips_through_pyyaml_compatible_quoting() {
        ...
        // legacy Python backend re-reading a Rust-written config would fail
