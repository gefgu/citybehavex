//! Mirrors `legacy Python backend/payload/legacy.py:1797-1857`'s
...
//! in `comparison::activity`; literature-basis constants/remapping and motif
//! SVG rendering come from `fastmob-vis`.
...
use fastmob_vis::motifs::{
    LITERATURE_TO_FASTMOB_MOTIF_ID, MotifBasisRow, MotifDistributionLike, format_motif_hex_id,
    literature_distribution_rows, map_motif_distribution_to_literature_basis,
};
use polars::prelude::*;
use serde_json::{Value, json};
use std::collections::BTreeMap;

impl MotifDistributionLike for MotifDistributionRow {
...
pub fn motif_axis_label_styles() -> (BTreeMap<String, String>, BTreeMap<String, Value>) {
    ...
    for (ordinal, &(_, fastmob_motif_id)) in LITERATURE_TO_FASTMOB_MOTIF_ID.iter().enumerate() {
        ...
        let svg = fastmob_vis::motif_svg::render_svg(fastmob_motif_id as u64).unwrap_or_default();
        let encoded = base64::Engine::encode(&base64::engine::general_purpose::STANDARD, svg);
...
fn basis_row_json(row: &MotifBasisRow) -> Value {
    json!({
        "literature_motif_id": serde_json::to_value(&row.literature_motif_id).unwrap_or(Value::Null),
        "motif_id": serde_json::to_value(&row.motif_id).unwrap_or(Value::Null),
        "hex_id": row.hex_id,
...
mod tests {
    use super::*;
    use fastmob_vis::motifs::OTHER_MOTIF_ID;
    ...
    fn literature_distribution_rows_matches_python_reference() {
        ...
        assert_eq!(
            serde_json::to_value(&rows[0].literature_motif_id).unwrap(),
            json!(1)
        );
        assert_eq!(
            serde_json::to_value(&rows[0].motif_id).unwrap(),
            json!(68719476736i64)
        );
        ...
        assert_eq!(
            serde_json::to_value(&other.literature_motif_id).unwrap(),
            json!(OTHER_MOTIF_ID)
        );
