...
/// JSON 404 for unmatched `/api/*` paths, matching legacy FastAPI's default
...
/// `@app.exception_handler(404)` is a Starlette/legacy FastAPI exception handler
...
/// legacy Python backend: `GET /api/experiments/{unknown}` returns
