//! On-disk cache for comparison payloads, mirroring `legacy Python backend/cache.py`.
//!
...
//! `config::cache_dir`), separate from the legacy Python backend's
...
mod tests {
    ...
    async fn refresh_forces_rebuild() {
        ...
        for _ in 0..2 {
            let calls = calls.clone();
            cache
                .get_or_build("exp", "run", &synthetic, None, &[], None, true, move || {
                    calls.fetch_add(1, Ordering::SeqCst);
                    Ok(json!({"n": 1}))
                })
                .await
