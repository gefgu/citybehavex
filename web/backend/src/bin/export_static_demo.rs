...
//!
//! Rust successor to `cargo run -p citybehavex-web --bin export_static_demo`, which called
//! `legacy Python backend`'s payload builders directly (in-process, not over
        ...
        .filter(|edge| {
            edge.as_array().is_some_and(|arr| {
                arr.len() >= 2
                    && arr[0]
                        .as_i64()
                        .is_some_and(|v| kept_zero_based.contains(&v))
                    && arr[1]
                        .as_i64()
                        .is_some_and(|v| kept_zero_based.contains(&v))
            })
...
fn sample_run(experiment: &Experiment, max_uid: i64) -> anyhow::Result<Experiment> {
    ...
    let sibling =
        |suffix: &str| sample_path.with_file_name(format!("{stem}_first{max_uid}{suffix}.{ext}"));
    ...
    if let Some(profiles_path) = &experiment.profiles_path {
        if profiles_path.exists()
            && duckdb_parquet_columns(profiles_path)?
                .iter()
                .any(|c| c == "uid")
        {
            let p_stem = profiles_path
                .file_stem()
                .unwrap_or_default()
                .to_string_lossy();
...
fn build_observed_graph_for_export(
    ...
    Some((|| {
        let df = citybehavex_web::comparison::trajectory::read_parquet(path)
            .map_err(|e| e.to_string())?;
        let cols: Vec<&str> = df.get_column_names().iter().map(|s| s.as_str()).collect();
        let uid_col =
            citybehavex_web::columns::detect_in(&cols, citybehavex_web::columns::UID_CANDIDATES);
        let datetime_col = citybehavex_web::columns::detect_in(
            &cols,
            citybehavex_web::columns::DATETIME_CANDIDATES,
        );
        let (Some(uid_col), Some(datetime_col)) = (uid_col, datetime_col) else {
            return Err(
                "observed network validation requires user and datetime columns".to_string(),
            );
        };
...
fn build_chart_payloads(
    ...
    let selected = experiment
        .runs
        .first()
        .expect("at least one run selected")
        .clone();
    ...
    for (section, filter_key) in sections {
        let Ok(mut section_payload) = payload::chart_section_payload(&ctx, section, filter_key)
        else {
            continue;
        };
    ...
    let mut network_validation = payload::network_validation_payload(
        ...
        observed_graph
            .as_ref()
            .map(|r| r.as_ref().map_err(|e| e.as_str())),
        experiment.network_validation_config.random_seed,
    );
    network_validation["run_id"] = json!(selected.run_id);
    write_json(
        &out_dir.join("network-validation.json"),
        &wrapped(network_validation),
    )?;
    ...
    write_json(
        &out_dir.join("home-work").join("all.json"),
        &wrapped(home_work_payload),
    )?;
...
fn export_timeline_chunks(
    ...
    write_json(
        &out_dir.join("timeline").join("meta.json"),
        &wrapped(meta.clone()),
    )?;
...
fn export_agent_details(
    ...
    let selected = experiment.runs.first().expect("at least one run selected");
    println!(
        "[{}] exporting agent details for {max_agents} agents",
        experiment.id
    );
...
fn export_static_demo(manifest_path: &Path) -> anyhow::Result<()> {
    ...
    write_json(
        &output_dir.join("experiments.json"),
        &wrapped(json!(experiments_payload)),
    )?;
    for (exp_id, detail) in &detail_payloads {
        write_json(
            &output_dir
                .join("experiments")
                .join(format!("{exp_id}.json")),
            &wrapped(detail.clone()),
        )?;
    }
